...
class LogsController extends AppController
{
    ...
    {
        $query = $this->Logs->find()->contain(['Users']);
        $search = $this->request->getQuery('message');
        if (!empty($search)) {
            $query->where(['message LIKE' => "%$search%"]);
        }
        if ($this->request->getQuery('sort')) {
            $query->Orderby([$this->request->getQuery('sort') => $this->request->getQuery('direction')]);
        }else{
            $query->Orderby(['createtime' => 'DESC']);
        }
        $logs = $this->paginate($query, ['limit' => 500]);
